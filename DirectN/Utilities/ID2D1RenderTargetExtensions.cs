...
namespace DirectN
{
    public static class ID2D1RenderTargetExtensions
    {
        ...
        public static void DrawGeometry(this IComObject<ID2D1RenderTarget> device,
            IComObject<ID2D1Geometry> geometry,
            IComObject<ID2D1Brush> brush,
            float strokeWidth,
            IComObject<ID2D1StrokeStyle> strokeStyle = null) => DrawGeometry(device?.Object, geometry?.Object, brush?.Object, strokeWidth, strokeStyle?.Object);

        public static void DrawGeometry(this ID2D1RenderTarget device,
            ...
            ID2D1StrokeStyle strokeStyle = null)
        {
            ...
            device.DrawGeometry(geometry, brush, strokeWidth, strokeStyle);
        }

        public static void PushAxisAlignedClip(this IComObject<ID2D1RenderTarget> device, D2D_RECT_F clipRect, D2D1_ANTIALIAS_MODE antialiasMode = D2D1_ANTIALIAS_MODE.D2D1_ANTIALIAS_MODE_PER_PRIMITIVE) => PushAxisAlignedClip(device?.Object, clipRect, antialiasMode);
        public static void PushAxisAlignedClip(this ID2D1RenderTarget device, D2D_RECT_F clipRect, D2D1_ANTIALIAS_MODE antialiasMode = D2D1_ANTIALIAS_MODE.D2D1_ANTIALIAS_MODE_PER_PRIMITIVE)
        {
            if (device == null)
                throw new ArgumentNullException(nameof(device));

            device.PushAxisAlignedClip(ref clipRect, antialiasMode);
        }

        public static void PopAxisAlignedClip(this IComObject<ID2D1RenderTarget> device) => PopAxisAlignedClip(device?.Object);
        public static void PopAxisAlignedClip(this ID2D1RenderTarget device)
        {
            if (device == null)
                throw new ArgumentNullException(nameof(device));
