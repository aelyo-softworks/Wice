...
/// </summary>
/// <remarks>
/// Instances are created/registered through the static <c>Define</c>/<c>Add</c> helpers, which forward to the
/// global property registry in <see cref="BaseObjectProperty"/> and then freeze the descriptor metadata.
/// The default invalidation for effect properties is <see cref="VisualPropertyInvalidateModes.Render"/>.
/// </remarks>
public class EffectProperty : VisualProperty
{
    ...
    /// <param name="mode">Invalidation modes triggered when the value changes; defaults to Render.</param>
    /// <param name="converter">Optional conversion hook invoked before storage.</param>
    /// <exception cref="ArgumentNullException">When <paramref name="declaringType"/> is null.</exception>
    ...
    /// </summary>
    /// <remarks>
    /// The value is mutable until the descriptor is frozen during registration; attempts to set after
    /// freezing will throw.
    /// </remarks>
